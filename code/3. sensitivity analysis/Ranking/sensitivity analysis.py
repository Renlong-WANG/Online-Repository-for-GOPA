...
# =========================
# Reference density v(x) and its normalized integral F(x)
# =========================
...
# =========================
# Normalize U_{ijr}: sum_r U = 1 for each (i, j)
# =========================
...
# =========================
# Read inputs (same format as your original)
# =========================
def read_inputs(
    xlsx_path: str,
    n_expert: int,
    ...
    # Alternative ranks: (n_expert*R) x n_attribute
    ...
    # Reference function type per (i,j)
    ...
    return t, s, alt_rank, cons_raw, ref_type


# =========================
# Baseline GOPA-RUE: compute U_ijr, z*, bar_w_ijr, and baseline WK using original ranks
# =========================
def compute_baseline_components(
    t: np.ndarray,
    s: np.ndarray,
    alt_rank: np.ndarray,
    cons_raw: np.ndarray,
    ...
    # Baseline mapping using original integer ranks
...
# =========================
# Ranking perturbation trial:
#   r_ijk -> r~N(r_ijk, sd), clip [1,R], then w = interp(r~, bar_w_ijr)
# =========================
def run_ranking_perturbation_trial(
    bar_w_ijr: np.ndarray,   # (I,J,R)
    alt_rank: np.ndarray,    # (I,J,R) integer ranks, mean of the normal
    sd: float,
    rng: np.random.Generator,
) -> Tuple[np.ndarray, np.ndarray]:
    I, J, R = bar_w_ijr.shape
    r_grid = np.arange(1, R + 1, dtype=float)

    # interpolate per (i,j,k) using sampled continuous rank
    w_ijk = np.zeros((I, J, R), dtype=float)

    # sample continuous perturbed ranks
    r_sample = rng.normal(loc=alt_rank.astype(float), scale=sd)
    r_sample = np.clip(r_sample, 1.0, float(R))

    for i in range(I):
        for j in range(J):
            fp = bar_w_ijr[i, j, :].astype(float)
            # np.interp: x, xp, fp
            w_ijk[i, j, :] = np.interp(r_sample[i, j, :], r_grid, fp)

    w_ijk = np.maximum(w_ijk, 0.0)
...
# =========================
def run_ranking_parameter_sensitivity(
    xlsx_path: str,
    n_expert: int,
    n_attribute: int,
    n_alternative: int,
    n_trials: int = 200,
    seed: int = 123,
    sds: Tuple[float, float, float] = (1/3, 1/2, 2/3),
    out_path: str = "ranking_sensitivity_output.xlsx",
) -> Dict[str, object]:
    I, J, R = n_expert, n_attribute, n_alternative

    t, s, alt_rank, cons_raw, ref_type = read_inputs(
        xlsx_path=xlsx_path,
        n_expert=I,
        n_attribute=J,
        n_alternative=R,
    ...
    base = compute_baseline_components(
        t=t, s=s, alt_rank=alt_rank, cons_raw=cons_raw, ref_type=ref_type,
        n_expert=I, n_attribute=J, n_alternative=R,
    )

    bar_w = base["bar_w_ijr"]
    W_base = base["W_K_base"]
    rank_base = base["rank_K_base"]
    ...
    results_all: Dict[str, Dict[str, object]] = {}
    ...
    with pd.ExcelWriter(out_path, engine="openpyxl") as writer:
        ...
        for sd in sds:
            ...
            for trial in range(n_trials):
                Wk, rk = run_ranking_perturbation_trial(
                    bar_w_ijr=bar_w,
                    alt_rank=alt_rank,
                    sd=float(sd),
                    rng=rng,
            ...
            df_long = pd.DataFrame(long_rows).sort_values(["trial", "rank", "alt_k"]).reset_index(drop=True)

            # wide tables
            df_wide_w = pd.DataFrame(W_trials, columns=[f"alt_{k}" for k in range(1, R + 1)])
            df_wide_w.insert(0, "trial", np.arange(1, n_trials + 1, dtype=int))

            df_wide_r = pd.DataFrame(rank_trials, columns=[f"alt_{k}" for k in range(1, R + 1)])
            df_wide_r.insert(0, "trial", np.arange(1, n_trials + 1, dtype=int))
            ...
            tag = f"SD_{sd:.3f}"
            # Excel sheet name max 31 chars, keep short
            df_long.to_excel(writer, sheet_name=f"{tag}_Long", index=False)
            df_wide_w.to_excel(writer, sheet_name=f"{tag}_WideWeight", index=False)
            df_wide_r.to_excel(writer, sheet_name=f"{tag}_WideRank", index=False)
            df_stats.to_excel(writer, sheet_name=f"{tag}_WeightStats", index=False)
    ...
    return {
        "baseline": {"W_K": W_base, "rank_K": rank_base},
        "results": results_all,
...
if __name__ == "__main__":
    ...
    res = run_ranking_parameter_sensitivity(
        xlsx_path="test.xlsx",
        n_expert=n_expert,
        n_attribute=n_attribute,
        n_alternative=n_alternative,
        n_trials=200,  # 你可以改成 1000
        seed=123,
        sds=(1/3, 1/2, 2/3),
        out_path="ranking_sensitivity_output.xlsx",
    )
